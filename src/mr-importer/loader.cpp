...
inline namespace importer {
   ...
   */
  static std::optional<AccessorDescription> get_accessor_by_name(
    Options options,
    const fastgltf::Asset &asset,
...
static void decode_draco_index_buffer(draco::Mesh *mesh, size_t component_size, std::vector<uint8_t> &out_buffer) {
    if (component_size == 4) {
...
template <typename T>
static bool get_attribute_for_all_points(draco::Mesh *mesh, const draco::PointAttribute *p_attribute, std::vector<uint8_t> &out_buffer) {
    size_t byte_offset = 0;
    T values[4] = {0, 0, 0, 0};
    for (draco::PointIndex i(0); i < mesh->num_points(); ++i) {
        const draco::AttributeValueIndex val_index = p_attribute->mapped_index(i);
        if (!p_attribute->ConvertValue<T>(val_index, p_attribute->num_components(), values)) {
            PANIC(T(), p_attribute->num_components(), p_attribute->attribute_type(), p_attribute->size(), values);
            return false;
        }
...
static bool get_attribute_for_all_points(fastgltf::ComponentType component_type, draco::Mesh *mesh, 
                                        const draco::PointAttribute *p_attribute, std::vector<uint8_t> &out_buffer) {
...
static size_t get_component_size_in_bytes(fastgltf::ComponentType component_type) {
    switch (component_type) {
        case fastgltf::ComponentType::Byte:
        case fastgltf::ComponentType::UnsignedByte:
            return 1;
                                  ...
                                  IndexArray& out_indices)
{
  if (!primitive.dracoCompression) {
    ...
    std::visit(fastgltf::visitor {
        [&](const fastgltf::sources::Array& array) {
          draco_data = {
            (const char *)array.bytes.data() + buffer_view.byteOffset,
    ...
    if (primitive.indicesAccessor.has_value()) {
        ...
        size_t component_size = get_component_size_in_bytes(indices_accessor.componentType);
    ...
    for (const auto& [attribute_name, draco_attribute_id] : primitive.dracoCompression->attributes) {
      ...
      if (attr_accessor == nullptr) continue;
      auto& accessor = asset.accessors[attr_accessor->accessorIndex];
      component_type = accessor.componentType;

      std::vector<uint8_t> attr_buffer;
      attr_buffer.resize(mesh->num_points() * draco_attr->num_components() * get_component_size_in_bytes(component_type));
  ...
  static std::optional<Mesh> get_mesh_from_primitive(Options options, const fastgltf::Asset &asset, const fastgltf::Primitive &primitive)
  {
    ...
    if (decode_draco_primitive(asset, primitive, mesh.positions, mesh.attributes, mesh.indices)) {
      ...
      tbb::parallel_invoke(
        [&]() {
          std::optional<AccessorDescription> positions = get_accessor_by_name(options, asset, primitive, "POSITION");
        ...
        [&]() {
          std::optional<AccessorDescription> normals = get_accessor_by_name(options, asset, primitive, "NORMAL");
          if (normals.has_value()) {
            ...
            fastgltf::iterateAccessorWithIndex<glm::vec3>(asset, normals.value().accessor, [&](glm::vec3 v, int index) {
              mesh.attributes[index].normal = {v.x, v.y, v.z};
            });
          }
        },
        [&]() {
          std::optional<AccessorDescription> texcoords = get_accessor_by_name(options, asset, primitive, "TEXCOORD_0");
          if (texcoords.has_value()) {
            ASSERT(texcoords.value().accessor.type == fastgltf::AccessorType::Vec2);
            mesh.attributes.resize(texcoords.value().accessor.count);
            fastgltf::iterateAccessorWithIndex<glm::vec2>(asset, texcoords.value().accessor, [&](glm::vec2 v, int index) {
    ...
    const fastgltf::Image &image)
  {
    ...
    auto load_dds_from_file = [](const std::string& path, ImageData& new_image) -> bool {
      ...
      if (res != dds::ReadResult::Success) return false;
      if (dds_image.data.get() == nullptr) return false;
      if (dds::getBitsPerPixel(dds_image.format) % 8 != 0) return false;
    ...
    auto load_dds_from_memory = [](const std::byte* data, size_t size, ImageData& new_image, const std::string& context) -> bool {
    ...
    auto load_ktx2_from_memory = [](const std::byte* data, size_t size, ImageData& new_image, const std::string& context) -> bool {
    ...
    auto try_load_wuffs_from_file = [](const std::string& path, ImageData& new_image) -> bool {
      ...
      if (!img.error_message.empty()) return false;
      if (!img.pixbuf.pixcfg.is_valid()) return false;

      wuffs_base__table_u8 tab = img.pixbuf.plane(0);
      wuffs_base__pixel_format format = img.pixbuf.pixel_format();

      if (format.bits_per_pixel() % 8 != 0) return false;
      ...
      if (new_image.width <= 0 || new_image.height <= 0 || new_image.bytes_per_pixel <= 0) return false;
    ...
    auto try_load_wuffs_from_memory = [](const std::byte* data, size_t size, ImageData& new_image, const std::string& context) -> bool {
    ...
    // Helper function to attempt WUFFS loading with fallback for ambiguous mime types
    auto try_load_with_fallback = [&](const std::byte* data, size_t size, 
    ...
    std::visit(
        ...
        ASSERT(filePath.fileByteOffset == 0,
            "Offsets with files are not supported because plain wuffs' C++ bindings don't support them.",
            filePath.uri.c_str());
        ...
        if (!try_load_with_fallback((const std::byte*)array.bytes.data(), 
              ...
              "memory array")) {
          PANIC("Failed to load image from memory array with all available methods");
        }
        },
        [&](const fastgltf::sources::Vector& vector) {
          ...
          if (!try_load_with_fallback((const std::byte*)vector.bytes.data(), 
                ...
                vector.mimeType, 
                "memory vector")) {
            PANIC("Failed to load image from memory vector with all available methods");
          }
        },
        [&](const fastgltf::sources::BufferView& view) {
          ...
          std::visit(fastgltf::visitor {
              [](auto& arg) { 
              ASSERT(false, 
                  "Try to process image from buffer view but not from RAM "
                  "(should be illegal because of LoadExternalBuffers)", arg); 
              },
              [&](const fastgltf::sources::Array& array) {
              ZoneScopedN("Import from buffer view array");
              const std::byte* data = (const std::byte*)array.bytes.data() + bufferView.byteOffset;
              if (!try_load_with_fallback(data, bufferView.byteLength, 
                    array.mimeType, 
                    "buffer view array")) {
              PANIC("Failed to load image from buffer view array with all available methods");
              }
              },
              [&](const fastgltf::sources::Vector& vector) {
              ZoneScopedN("Import from buffer view vector");
              const std::byte* data = (const std::byte*)vector.bytes.data() + bufferView.byteOffset;
              if (!try_load_with_fallback(data, bufferView.byteLength, 
                    vector.mimeType, 
                    "buffer view vector")) {
                PANIC("Failed to load image from buffer view vector with all available methods");
              }
              }
          }, buffer.data);
        },
  ...
  void add_loader_nodes(FlowGraph &graph, const Options &options)
  {
    ...
    graph.meshes_load = std::make_unique<tbb::flow::function_node<fastgltf::Asset*, fastgltf::Asset*>>(
      graph.graph, tbb::flow::unlimited, [&graph, &options](fastgltf::Asset* asset) -> fastgltf::Asset* {
        if (asset != nullptr) {
          ZoneScoped;
